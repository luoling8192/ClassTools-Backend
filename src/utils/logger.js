...
module.exports = print = {
    ...
    warn: ($message) => {
        let date = getDate();
        console.warn(`[${date}]${getCaller()}${chalk.yellow(`[WARN] ` + $message)}`);
    },
    error: ($message) => {
        let date = getDate();
        if (typeof($message) === "object")
            console.error(`[${date}]${getCaller()}${chalk.red(`[ERROR] ${$message.stack}`)}`);
        else
            console.error(`[${date}]${getCaller()}${chalk.red(`[ERROR] ` + $message)}`);
    },
    fatal: ($message) => {
        let date = getDate();
    ...
    success: ($message) => {
        let date = getDate();
        console.log(`[${date}]${getCaller()}${chalk.blue("[INFO]")} ${chalk.green($message)}`);
    },
    message: ($message) => {
        let date = getDate();
